...
In general, every character is represented as a pair of the ASCII character and a list of identifiers where each identifier consists of a decimal number and a site ID. A site ID is a unique number associated with a client. For example, say there are two users (# and $) and they perform the operation in the following order:
1. User # types 'C'.
